...
const (
	...
	diapazon      = "Числа должны быть от 1 до 10 включительно"
)
...
func ConvToRome(number int) string {
	...
	// Срез значений для итерации в порядке убывания
	for _, d := range []int{100, 90, 50, 40, 10, 9, 5, 4, 1} {
		// пока наше число больше или равно текущему значению из среза
		for number >= d {
			roman += ToRome[d] // добавляем соответствующую римскую цифру к результату
			number -= d        // вычитаем добавленное число из изначального
		}
	}
	return roman
}
...
func main() {
	...
	if Numeral1 < 1 || Numeral1 > 10 || Numeral2 < 1 || Numeral2 > 10 {
		panic(diapazon)
	}
	var res int
	switch operator {
